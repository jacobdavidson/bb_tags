...
# Encode a customized version of the hamming code with general parity bit, a blockLength from 16 and a msgLength of 11  
def hamming_16_11_encode(bin11):
    # First parity bit
    E1 = parity(bin11[0] + bin11[1] + bin11[3] + bin11[4] + bin11[6] + bin11[8] + bin11[10], 'even')
    # Second parity bit
    E2 = parity(bin11[0] + bin11[2] + bin11[3] + bin11[5] + bin11[6] + bin11[9] + bin11[10], 'even')
    # Third parity bit
    E4 = parity(bin11[1] + bin11[2] + bin11[3] + bin11[7] + bin11[8] + bin11[9] + bin11[10], 'even')
    # Fourth parity bit
    E8 = parity(bin11[4] + bin11[5] + bin11[6] + bin11[7] + bin11[8] + bin11[9] + bin11[10], 'even')
    # Global parity bith
    P  = parity(E1 + E2 + bin11[0] + E4 + bin11[1:4] + E8 + bin11[4:11], 'even')
    
    hamming = E1 + E2 + bin11[0] + E4 + bin11[1:4] + E8 + bin11[4:11] + P
...
def drawTag (s,x,cx,cy,rC,SWC,rA,rO,SWO):
    ...
    while a < len(x):
        ...
        if a == 0: #1st bit            
            qsx = cx - rA
            qsy = cy
            qex = (1 - math.cos(pi2/8))*rA
            qey = -math.sin(pi2/8)*rA
        elif a == 1: #2nd bit
            qsx = cx - math.cos(pi2/8)*rA
            qsy = cy - math.sin(pi2/8)*rA
            qex = (math.cos(pi2/8) - math.cos(pi2/4))*rA
            qey = -(math.sin(pi2/4) - math.sin(pi2/8))*rA
        elif a == 2: #3rd bit
            qsx = cx - math.cos(pi2/4)*rA
            qsy = cy - math.sin(pi2/4)*rA
            qex = (math.cos(pi2/4) - math.cos(3*pi2/8))*rA
            qey = -(math.sin(3*pi2/8) - math.sin(pi2/4))*rA
        elif a == 3: #4th bit
            qsx = cx - math.cos(3*pi2/8)*rA
            qsy = cy - math.sin(3*pi2/8)*rA
            qex = math.cos(3*pi2/8)*rA
            qey = -(1 - math.sin(3*pi2/8))*rA
...
for ind in range (0,numT):
    ...
    #The ID in Hamming code (16,11)
    markBin2= hamming_16_11_encode(markBin)    
    #The marker is printed
    drawTag(s,markBin2,cx,cy,rC,SWC,rA,rO,SWO)

#the set of Tags that was generated
t = Text("Bee Hamming code (16,11) Even Parity " + str(numT) + " Tags, " + str(canvasW) + " x " + str(canvasH) + " mm" , 75, canvasH*2-2)
t.set_font_size(8)

s.addElement(t)

#the file is saved
s.save('./testOutput/Bee16bitsEven.svg', encoding='UTF-8')
